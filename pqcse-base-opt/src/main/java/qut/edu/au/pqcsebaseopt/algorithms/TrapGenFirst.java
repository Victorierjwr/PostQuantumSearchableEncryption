...
import static qut.edu.au.pqcsebaseopt.tools.HermiteNormalForm.hermiteNormalForm;
...
public class TrapGenFirst {
    ...
    public BigIntMatrix genA1() {
        ...
        if (BigIntMatrix.image(A1).getRows() != n_long) {
    ...
    public BigIntMatrix genH(BigIntMatrix matrix) {
        ...
        BigIntMatrix hnfRaw = hermiteNormalForm(matrix);

        // Wrap back to BigIntMatrix
        BigIntMatrix H = new BigIntMatrix(hnfRaw.getRows(), hnfRaw.getCols(), q);
        for(int i=0; i<hnfRaw.getRows(); i++) {
            for(int j=0; j<hnfRaw.getCols(); j++) {
                H.set(i, j, hnfRaw.get(i,j));
            }
        }
        return H;
    }
    ...
    public void print(String name, BigIntMatrix matrix) {
        System.out.println(name + " dims: {" + matrix.getRows() + ", " + matrix.getCols() + "}");
        // Avoid printing overly large matrices
        if (matrix.getRows() <= 1000 && matrix.getCols() <= 1000) {
